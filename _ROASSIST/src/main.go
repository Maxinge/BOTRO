...
func mouseProc(nCode int, wParam uintptr, lParam uintptr) uintptr {
	if nCode == 0 {
		...
		case 524:
            ...
            MUgroundItems.Lock()
            for kk,_ := range groundItems {
                targetItemID = kk
                it := groundItems[targetItemID];
               
                if timers.TclickLoot <= 0 {

                    if int(math.Round(getDist(charCoord,it.Coords))) <= 2{
                        itemBin := make([]byte, 4) ;
                    ...
                    if int(math.Round(getDist(charCoord,it.Coords))) > 2{
                    if int(math.Round(getDist(charCoord,it.Coords))) <= 7{
		...
		case 522:
			// fmt.Println("aaa") // molette

		case WM_LBUTTONDOWN:
			// fmt.Println("Bouton gauche de la souris appuyé")
		case WM_RBUTTONDOWN:
			// fmt.Println("Bouton droit de la souris appuyé")
		}
	}
	ret, _, _ := callNextHookEx.Call(0, uintptr(nCode), wParam, lParam)
	return ret
...
func main() {
    ...
    go func() {
        ...
        keyboardHook, _, err := setWindowsHookEx.Call(
            WH_KEYBOARD_LL,
            syscall.NewCallback(keyboardProc),
            0,
            0,
        )
        if keyboardHook == 0 { fmt.Printf("err setWindowsHookEx : %v\n", err); return }
        ...
        if mouseHook == 0 { fmt.Printf("Impossible setWindowsHookEx : %v\n", err); return }
