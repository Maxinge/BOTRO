...
func parsePacket(bb []byte){
    ...
    case "01DE", "09CB", "08C8":  //skill_used_on_target //skill_no_dmg //actor_action
        ...
        if hexID == "08C8" {
        if sourceID == accountID && dmg > 0{
            targetMobID = targetID
        }}


        if autoSkillAspd > 0 {
            MUmobList.Lock()
            if _, exist := mobList[targetID]; exist {
            if sourceID == accountID {
            if hexID == "01DE"{
                HitPS := float32(50)/(float32(200)-float32(autoSkillAspd))
                TBA := int(((1/HitPS)*1000)*0.5)
                go func() {
                    skillID := int(binary.LittleEndian.Uint16(bb[0:0+2]))
                    skillLv := int(binary.LittleEndian.Uint16(bb[26:26+2]))
                    if skillID == 219 { return } // snatch
                    // fmt.Printf("### skillID ### [%v] \n",skillID )
                    // fmt.Printf("### skillLv ### [%v] \n",skillLv )
                    // fmt.Printf("### sourceID ### [%v] \n",sourceID )
                    // fmt.Printf("### targetID ### [%v] \n",targetID )
                    time.Sleep(time.Duration(TBA) * time.Millisecond)
                    sendUseSkill(skillID, skillLv, targetID)
                }()
            }}}
            MUmobList.Unlock()
        }
