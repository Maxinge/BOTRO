	...
	for {
		...
		for _,vect := range beamDirections {
			...
			for _,beamCell := range beamCellList {
				...
				if beamCell.X < 0 || beamCell.Y < 0 { break badbeam }
				gatCell := lgatMap.cells[beamCell.X][beamCell.Y]
				if !isValidCell(gatCell) { break badbeam }
				betweens := linearInterpolation(beamCell, coordList[k])
		...
		if best > 0{
			...
			temp := []Coord{}
			temp = append(temp,coordList[:k+1]...)
			temp = append(temp,line[:len(line)-1]...)
			temp = append(temp,coordList[best:]...)
			coordList = temp
			k++
		}
	}
